...
## Hazards encountered
- **Auto-push will commit large files before `.gitignore` has a chance to catch them.** The launchd auto-push job runs every 30s; if a new large file appears between two runs, it'll be committed. Once it's in history, `git push` to GitHub will fail (>100 MB rejects with `GH001: Large files detected`). For this project the TinyStories file (181 MB) was auto-committed twice, then auto-committed again after the third run. Fix: `.gitignore` the artifact directory *before* generating files there, OR temporarily set `auto_log.enabled: false`-equivalent on the auto-push job during long-running data work. Recovery for already-committed: `git reset --mixed origin/main` (assumes commits unpushed), `git rm --cached <bigfile>`, ensure `.gitignore` catches it, then a single clean commit + push.
- The auto-push had successfully pushed an earlier 76 KB version of the data file BEFORE it grew. So origin/main had a partial bad file even after the 100 MB threshold started failing later pushes. Lesson: any artifact-style file should be in `.gitignore` from the moment the directory is created, even if it's currently small.
